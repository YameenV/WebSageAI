...
class WebScraper:
    ...
    def __clean_html_content(
        ...
        '''
        fetch and clear text from html
        '''
    ...
    async def scraping_with_langchain(
        ...
        '''
        Load the text in with langchain AsyncHtmlLoader and return a clean text from the url 
        '''
        try:
            loader = AsyncHtmlLoader(
                [self.url],
                verify_ssl=False,  # Disable SSL verification
                raise_for_status=True,  # Raise an exception for bad status codes
                ignore_load_errors=False,  # Don't ignore load errors
            )
            docs = loader.load()
